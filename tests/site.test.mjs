...
test("homepage is a chronological release grid with stable project routing", async () => {
  ...
  assert.match(home, /<dt>Online script<\/dt>\s*<dd>54,316 Japanese\/English lines<\/dd>/);
  ...
  assert.match(home, /English translation · v2\.1\.0/);
  ...
  assert.equal((home.match(/<figure class="testimonial">/g) ?? []).length, 9);
  assert.match(home, /Well, it looks kino\./);
  assert.match(home, /https:\/\/boards\.4chan\.org\/vg\/thread\/582129626#p582178381/);
  ...
  assert.equal((home.match(/on\s+MAO’s English\s+translation of/g) ?? []).length, 8);
...
test("OreTsuba release card links the published game and reader",async()=>{
 ...
 assert.match(home,/data-release-repo="oretsuba">English translation · v1\.1\.3/);
