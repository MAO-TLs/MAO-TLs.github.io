...
test("homepage is a chronological release grid with stable project routing", async () => {
  ...
  assert.match(home, /class="release-link" href="\/fate-stay-night\/script\/">Read script<\/a>/);
...
test("release facts use the same labels and stay attached to their action", async () => {
  ...
  for (const [card] of cards) {
    if (card.includes('release-card-fate')) {
      ...
      assert.match(card, /<\/dl>\s*<a class="release-link" href="\/fate-stay-night\/script\/">Read script<\/a>/);
      continue;
    }
    assert.deepEqual([...card.matchAll(/<dt>(.*?)<\/dt>/g)].map((m) => m[1]), ["Includes", "Online script", "Runs on"]);
